...
> [!SOURCE] Sources:
> - *[alex996/presentations/auth.md](https://github.com/alex996/presentations/blob/master/auth.md?plain=1)*

> [!TIP] See Also
> - [API](../04-RESOURCES/Definitions/Acronyms/Application%20Programming%20Interface%20(API).md)
> - [Authentication](../04-RESOURCES/Definitions/Dictionary/Definition%20-%20Authentication.md)
> - [Authorization](../04-RESOURCES/Definitions/Dictionary/Definition%20-%20Authorization.md)
...
2. Server Verifies Credentials against Stored Database Records
...
7. When User Logs Out, Server Deletes the Session and Clears the Cookie
...
sequenceDiagram
    ...
    alt Credentials valid
        ...
        loop User requests
            User->>Browser: Initiates request
            Browser->>Server: Sends request with cookie
            Server->>Server: Validate session ID
            Server-->>Browser: Grants access
        ...
        Server->>Browser: Authentication failed
        Browser-->>User: Login error message
    end
...
flowchart TD
    ...
    B[Server verifies credentials against DB]
    C{Are credentials valid?}
    D[Server creates temporary session]
    E[Server issues cookie with session ID]
    F[User sends cookie with each request]
    G[Server validates session ID against session store]
    H[Grant access to user]
    I[User logs out]
    J[Server destroys session\nand clears cookie]
    K[Access denied]
    End[End]
...
- Every User Session is Stored Server-Side (**Stateful**)
  - Memory (i.e. File System)
  - Cache (i.e. `Redis` or `Memcached`)
  - Database (i.e. `Postgres`, `MongoDB`)
- Each User is Identified by a Session ID
  - **Opaque** Reference
    - No third-party can extract any data out
    - Only the Issuer (Server) is able to map back to data
  - Stored in a Cookie
    - Signed with a **Secret**
    - Protected with **Flags**
- [[Server Side Rendering (SSR)]] Web Apps, Frameworks (`Spring`, `Rails`), Scripting Languages (`PHP`)
...
```dataview
LIST FROM [[About Authentication]] AND -"CHANGELOG" AND -"00-INBOX/About Authentication"
```
